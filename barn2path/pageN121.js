const localVersion = 12.1;
// Code Version (barn2path will attempt to be backwards compatible with previous versions.)
// Should be kept in sync with pageN# in GitHub repo/lib.
// e.g. "5.2" == pageN52.js
...
let width = 1470; // pixels
let height = 778; // pixels
const xOffset = -27.5; // cm
const yOffset = -20; // cm
...
const previewImageScaleFactor = 0.77; // 77% of the original size
const jpegExportPreviewQuality = 0.77; // 77% quality
const fileExtension = ".b2path";
const dataType = DataTypes.JSON;
const encodingType = EncodingTypes.BARN2;
...
function setCanvasSizes() {
	...
	$("#field, #background, #canvases, #interactive").css({
		width: widthString,
		height: heightString,
	});
}
...
function loadImageBasedOnField(fieldName) {
	...
	switch (fieldName) {
		case "crescendo":
			src =
				"https://uploads-ssl.webflow.com/645accb94a84d4211340007c/65c65b2301d714a18f2faac1_FieldResized.png";
			break; // Added missing break statements
		case "charged up":
			src =
				"https://uploads-ssl.webflow.com/645accb94a84d4211340007c/65a32f800d444dfb5d1c8924_season.jpg";
			break;
		default:
			src =
				"https://uploads-ssl.webflow.com/645accb94a84d4211340007c/65c65b2301d714a18f2faac1_FieldResized.png";
...
function setupUIEventListeners() {
	...
	durationSelector.oninput = () =>
		(duration = Math.min(parseInt(durationSelector.value), 15)); // Limit duration to 15 seconds.
...
function init(fieldName) {
	...
	_addPoint(200, 200, 0, "Autonomous Start Pose", "start", false);
}
...
function addPoint() {
	...
	if (waypoints.length > 0)
		prev = waypoints[waypoints.length - 1].translation;
...
function _addPoint(
	...
	$("tbody").append(
		"<tr>" +
			"<td class='drag-handler'><i class='material-icons'>===</i></td>" +
			...
			"<td class='delete'><button onclick='deletePoint($(this).parent().parent());' class='icon-button'><i class='material-icons'>x</i></button></td></tr>"
	);

	lastAction.push({
		action: "add",
		indexInTable: waypoints.length,
		data: {
			translation: new Translation2d(x, y),
			rotation: Rotation2d.fromDegrees(heading),
		},
	});
...
function modifyWaypoint(index, x, y, wasProgrammatic = false) {
	...
	if (!wasProgrammatic) {
		lastAction.push({
			...
			indexInTable: index,
			data: {
				translation: new Translation2d(xInput.value, yInput.value),
				rotation: waypoints[index].rotation,
			},
		});
	}
...
function deletePoint(elm) {
	...
	for (let i = 0; i < oldWaypoints.length; i++) {
		if (!waypoints.includes(oldWaypoints[i])) {
			deletedWaypoints.push(oldWaypoints[i]);
		}
	}
	...
	lastAction.push({
		action: "delete",
		data: {
			translation: deletedWaypoints[0].translation,
			rotation: deletedWaypoints[0].rotation,
		},
	});
}

function undoLastAction() {
	const lastActionThis = lastAction.pop();
	if (lastActionThis.action === "none") {
		return;
	}
	redoStack.push(lastActionThis);
	switch (lastActionThis.action) {
		case "add":
			// remove whichever point was added
			$("tbody").children("tr").last().remove();
			update();
			break;
		case "modify":
			modifyWaypoint(
				lastActionThis.indexInTable,
				lastActionThis.data.translation._x,
				lastActionThis.data.translation._y,
				true
			);
			break;
		case "delete":
			_addPoint(
				lastActionThis.data.translation._x,
				lastActionThis.data.translation._y,
				lastActionThis.data.rotation.degrees
			);
			break;
	}
}
...
function drawRobot(position, heading, scaleFactor = 0.78) {
	...
	angles.forEach(function (angle) {
		...
		points.push(point);
		let color =
			Math.abs(angle - heading) < Math.PI / 2
				? Colors.LIGHT_BLUE
				: getNextColor();
	...
	$("tbody")
		...
		.each(function () {
			...
			let heading = Math.round(
				parseInt($($($(this).children()).children()[3]).val())
			);
			if (isNaN(heading)) {
				heading = 0;
			}
			...
			let enabled = $($($(this).children()).children()[6]).prop(
				"checked"
			);
			if (enabled) {
				waypoints.push(
...
function interpolatePose2D(p0, p1, cp, t) {
	let interpolatedTranslation = quadraticBezier(
		p0.translation,
		cp,
		p1.translation,
		t
	...
	let startRadians = cosSinToRadians(
		p0.rotation._cos_angle_,
		p0.rotation._sin_angle_
	);
	let endRadians = cosSinToRadians(
		p1.rotation._cos_angle_,
		p1.rotation._sin_angle_
	);
	let midRadians = (startRadians + endRadians) / 2;
	let interpolatedRadians = quadBezierInterpolate(
		startRadians,
		midRadians,
		endRadians,
		t
	...
	return new Pose2d(
		new Translation2d(
			interpolatedTranslation._x,
			interpolatedTranslation._y
		),
		{
...
function catmullRomInterpolate(p0, p1, p2, p3, t) {
	...
	let x =
		(2 * p1._x - 2 * p2._x + v0 + v1) * t3 +
		(-3 * p1._x + 3 * p2._x - 2 * v0 - v1) * t2 +
		v0 * t +
		p1._x;
	...
	v1 = (p3._y - p1._y) / 2;
	let y =
		(2 * p1._y - 2 * p2._y + v0 + v1) * t3 +
		(-3 * p1._y + 3 * p2._y - 2 * v0 - v1) * t2 +
		v0 * t +
		p1._y;
...
function extendLine(p1, p2, extensionFactor = 1.0) {
	...
	return {
		translation: {
			_x:
				p2.translation._x +
				extensionFactor * (p2.translation._x - p1.translation._x),
			_y:
				p2.translation._y +
				extensionFactor * (p2.translation._y - p1.translation._y),
		},
...
function calculateIntermediatePoints(inputPoints, density = pointDensity) {
	...
	if (inputPoints.length === 2) {
		let result = [];
		let startRotation = normalizeAngle(
			cosSinToRadians(
				inputPoints[0].rotation._cos_angle_,
				inputPoints[0].rotation._sin_angle_
			)
		);
		let endRotation = normalizeAngle(
			cosSinToRadians(
				inputPoints[1].rotation._cos_angle_,
				inputPoints[1].rotation._sin_angle_
			)
		);
		...
		const numIntermediatePoints = Math.max(
			Math.ceil(segmentDistance * density),
			1
		);

		for (let i = 0; i <= numIntermediatePoints; i++) {
			let t = i / numIntermediatePoints;
			let interpolatedTranslation = catmullRomInterpolate(
				inputPoints[0].translation,
				inputPoints[0].translation,
				inputPoints[1].translation,
				inputPoints[1].translation,
				t
			);
			let interpolatedRadians = normalizeAngle(
				quadBezierInterpolate(
					startRotation,
	...
	let startPoint =
		inputPoints.length < 3
			? extendLine(secondPoint, firstPoint, -1.0)
			: firstPoint;
	let endPoint =
		inputPoints.length < 3
			? extendLine(secondLastPoint, lastPoint, 1.0)
			: lastPoint;
	...
	let startRotation = normalizeAngle(
		cosSinToRadians(
			extendedPoints[0].rotation._cos_angle_,
			extendedPoints[0].rotation._sin_angle_
		)
	);
	...
	result.push(
		new Pose2d(
			new Translation2d(
				extendedPoints[0].translation._x,
				extendedPoints[0].translation._y
			),
			{
	...
	for (let i = 1; i < extendedPoints.length - 1; i++) {
		let currentRotation = normalizeAngle(
			cosSinToRadians(
				extendedPoints[i].rotation._cos_angle_,
				extendedPoints[i].rotation._sin_angle_
			)
		);
		let nextRotation = normalizeAngle(
			cosSinToRadians(
				extendedPoints[i + 1].rotation._cos_angle_,
				extendedPoints[i + 1].rotation._sin_angle_
			)
		);

		if (
			currentRotation !== nextRotation ||
			i === extendedPoints.length - 2
		) {
			...
			for (let j = segmentStartIndex; j <= i; j++) {
				...
				let p2 =
					extendedPoints[Math.min(j + 1, extendedPoints.length - 1)];
				let p3 =
					extendedPoints[Math.min(j + 2, extendedPoints.length - 1)];

				const segmentDistance = calculateDistance(p1, p2);

				let numIntermediatePoints = Math.max(
					Math.ceil(segmentDistance * density),
					1
				);
				...
				for (let k = 0; k <= numIntermediatePoints; k++) {
					...
					let segmentProgress =
						(j - segmentStartIndex + t) /
						(i - segmentStartIndex + 1);
					let interpolatedRadians = normalizeAngle(
						quadBezierInterpolate(
							startRotation,
							controlRotation,
					...
					let interpolatedRotation =
						radiansToCosSin(interpolatedRadians);
	...
	for (const point of result) {
		if (
			lp === null ||
			point.translation._x !== lp.translation._x ||
			point.translation._y !== lp.translation._y
		) {
...
function calculateAndParseSplines(inputPoints) {
	let result = calculateIntermediatePoints(inputPoints, pointDensity);
	return result;
}
...
function recalculateSplines(waypointsList, drawStyle) {
	if (isReversedCheckbox == null) {
		isReversedCheckbox = document.getElementById("Reverse-Path");
	}
	const orderedWaypoints = isReversedCheckbox.checked
		? waypointsList.slice(0).reverse()
		: waypointsList;
	const data = orderedWaypoints
		.map(
			(point) =>
				`${point.translation.x},${point.translation.y},${Math.round(
...
function drawSplines(fill, animate) {
	...
	if (animate) {
		...
		function animLoop() {
			...
			fillRobot(
				splinePoint,
				splinePoint.rotation.radians,
				"hsla(" + hue + ", 100%, 50%, 0.015)"
			);
		...
		splinePoints.forEach((splinePoint) => {
			...
			if (fill) {
				if (isReversedCheckbox == null) {
					isReversedCheckbox =
						document.getElementById("Reverse-Path");
				}
				let index = isReversedCheckbox.checked
...
function shfff(inStr, x, de) {
	...
	if (de) {
		...
		return (
			inStr.substring(inStr.length - x) +
			inStr.substring(0, inStr.length - x)
		);
	}
}
...
async function base64Decode(base64String) {
	...
	// Use fetch to retrieve the data URL as a Blob
...
async function fullEncrypt(data) {
	let temp = data;
	temp = await base64Encode(temp);
...
async function fullDecrypt(data) {
	let temp = data;
	temp = temp.replace(/\s*<\/?barn2path>\s*/g, "");
	temp = temp
		.replace(/\$/g, "A")
		.replace(/\!/g, "M")
		.replace(/\@/g, "C")
...
async function makeData() {
	...
	for (let i = 0; i < orderedWaypoints.length; i++) {
		for (let j = 0; j < smoothedPoints.length; j++) {
			if (
				roundOff(orderedWaypoints[i].translation._x, 2) ===
					roundOff(smoothedPoints[j].translation._x, 2) &&
				roundOff(orderedWaypoints[i].translation._y, 2) ===
					roundOff(smoothedPoints[j].translation._y, 2)
			) {
	...
	smoothedPoints = smoothedPoints.filter(
		(p) => !isNaN(p.translation._x) && !isNaN(p.translation._y)
	);
	let dataForm = makeWaypointsList(smoothedPoints);
	console.log(dataForm);
	// await the promise and get the result then pass it in
	//dataForm = await fullEncrypt(dataForm);
	return dataForm;
}
...
async function downloadPath(name) {
	...
	a.href = url;
	a.download = (name ? name : "routine") + fileExtension;
...
function generatePointsFromFile() {
	...
	waypoints = [];
	$("tbody").empty();
	pointsToGenerate.forEach((waypoint) => {
...
function makeWaypointsList(waypoints) {
    ...
    */
	...
	mergedCanvas.width = width * previewImageScaleFactor;
	mergedCanvas.height = height * previewImageScaleFactor;
	let mergedCtx = mergedCanvas.getContext("2d");

	// Assuming you've already created ctxBackground and fieldCanvas elsewhere
	// Scale and draw the background canvas
	mergedCtx.drawImage(
		ctxBackground.canvas,
		0,
		0,
		ctxBackground.canvas.width,
		ctxBackground.canvas.height,
		0,
		0,
		mergedCanvas.width,
		mergedCanvas.height
	...
	// Scale and draw the field canvas
	mergedCtx.drawImage(
		fieldCanvas,
		0,
		0,
	...
	img.src = url;
	let pathPreview = mergedCanvas.toDataURL(
		"image/jpeg",
		jpegExportPreviewQuality
	);
	pathPreview = pathPreview.replace("data:image/jpeg;base64,", "");
	switch (dataType) {
		case "json":
			return (
				'{\n\t"version": "' +
				...
				waypoints
					.map((waypoint, i) => {
						return (
							'{\n\t\t\t"x": ' +
							(waypoint.translation.x() / 100.0).toFixed(9) +
							',\n\t\t\t"y": ' +
							(waypoint.translation.y() / 100.0).toFixed(9) +
							',\n\t\t\t"r": ' +
							waypoint.rotation.degrees.toFixed(9) +
							',\n\t\t\t"e": "' +
							(i === 0
								? "start"
...
function importFile(data) {
	if (parseFloat(data.version).toFixed(1) >= 11.3 && parseFloat(data.version).toFixed(1) < 12.0) {
