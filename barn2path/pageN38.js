...
function init(fieldName) {
    ...
    switch (fieldBackground) {
        case "crescendo":
            src = "https://uploads-ssl.webflow.com/645accb94a84d4211340007c/65c545a55512de1d3fbf6301_NewField.png";
        case "charged up":
...
function makeWaypointsList(waypoints) {
    ...
    */
    ...
    mergedCanvas.canvas.width = width;
    mergedCanvas.canvas.height = height;
...
function loadWaypoints(csvString) {
    const lines = csvString.split('\n');
    // Remove header
    lines.shift();

    // Empty the current waypoints in the HTML table body
    $('tbody').empty();

    for(let i = 0; i < lines.length; i++) {
        if(!lines[i] == "") {
            let content = lines[i].split(",");
            let comment = content[3].replace("\"", "").slice(0, -1);
            let event = content[4].replace("\"", "").slice(0, -1);
            console.log(comment, event);
            try {
            _addPoint(
                parseFloat(content[0]),
                parseFloat(content[1]),
                parseFloat(content[2]), 
                comment,
                event
            );
            isReversedCheckbox.checked = JSON.parse(content[5].toLowerCase());
            } catch(e) {}
        }
    }
...
function waypointsToCsv(waypoints) {
    // Start with the header row
    let csvContent = "x,y,rotation,comment,event,reversed\n";
    
    // Append each waypoint data
    let pathPoints = document.getElementById("pathPoints");
    for (let i = 0; i < pathPoints.rows.length; i++) {
        let row = pathPoints.rows[i];
        const comment = row.getElementsByClassName("comments")[0].getElementsByTagName("input")[0].value;
        waypoints[i].comment = comment;
        const event = row.getElementsByClassName("event")[0].getElementsByTagName("input")[0].value;
        waypoints[i].event = event;
    }
    waypoints.forEach(waypoint => {
        const x = waypoint._translation_._x;
        const y = waypoint._translation_._y;
        // Calculate rotation in degrees
        const rotation = r2d(Math.atan2(waypoint._rotation_._sin_angle_, waypoint._rotation_._cos_angle_));
        const comment = waypoint.comment.replace(/,/g, ''); // Remove commas from comments to preserve CSV format
        const event = waypoint.event;
        csvContent += `${x},${y},${rotation},"${comment}","${event}",${isReversedCheckbox.checked}\n`;
    });
...
function generateCSV() {
    return waypointsToCsv(waypoints);
}
