...
function encodeMessageInImage(imageUrl, message) {
    const image = new Image();
    image.crossOrigin = "Anonymous"; // Needed for cross-origin images
    image.onload = function() {
        const cvas = document.getElementById("saveCanvas");
        ...
        // Loop through each pixel to modify
        for (let y = 0; y < cvas.height; y++) {
            for (let x = 0; x < cvas.width; x++) {
                if (dataIndex < binaryMessage.length) {
                    const pixelData = ctx.getImageData(x, y, 1, 1);
                    const color = pixelData.data;

                    // Modify the LSB of the blue channel to store one bit of the message
                    color[2] = (color[2] & ~1) | parseInt(binaryMessage[dataIndex], 2);
        ...
        // Output the modified image as a new Data URL
        const outputImageSrc = cvas.toDataURL();
...
async function downloadPath(name) {
    ...
    let finalEncoded = encodeMessageInImage("https://assets-global.website-files.com/645accb94a84d4211340007c/659a309783f545731ebc7d2c_barn2favicon_256.png", data);
...
function decodeMessageFromImage(file) {
    ...
    reader.onload = function(event) {
        ...
        image.onload = function() {
            ...
            const message = binaryMessage.match(/.{1,8}/g)
...
async function loadFromFile(file) {
    ...
    let decoded = decodeMessageFromImage(file);
