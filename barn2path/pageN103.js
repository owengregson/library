...
function base64Decode(base64String) {
    // Create a function to decode a base64-encoded string to a Uint8Array.
    const decodeBase64 = (str) => {
        const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
        let bytes = [];

        str = str.replace(/=+$/, ''); // Remove any trailing '='s
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const byteArray = decodeBase64(base64String);
...
async function fullEncrypt(data) {
    ...
    temp = temp.replace(/A/g, "$").replace(/M/g, "!").replace(/C/g, "@").replace(/D/g, "#").replace(/w/g, "&").replace(/j/g, "*").replace(/I/g, "^").replace(/i/g, "%").replace(/g/g, "<").replace(/k/g, ">").replace(/S/g, "?");
...
async function fullDecrypt(data) {
    let temp = data;
    temp = temp.replace(/\$/g, "A").replace(/\!/g, "M").replace(/\@/g, "C").replace(/\#/g, "D").replace(/\&/g, "w").replace(/\*/g, "j").replace(/\^/g, "I").replace(/\%/g, "i").replace(/</g, "g").replace(/>/g, "k").replace(/\?/g, "S");
    temp = shfff(temp, 751, true);
    temp = await base64Decode(temp);
    temp = temp.replace(/<barn2path> /g, "").replace(/ <\/barn2path>/g, "");
    return temp;
}
...
function makeWaypointsList(waypoints) {
    ...
    */
    ...
    img.src = url;
    let pathPreview = mergedCanvas.toDataURL('image/jpeg', jpegExportPreviewQuality);
