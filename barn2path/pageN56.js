    ...
    for (let i = 1; i < extendedPoints.length - 1; i++) {
        ...
        if (currentRotation !== nextRotation || i === extendedPoints.length - 2) {
            ...
            for (let j = segmentStartIndex; j <= i; j++) {
                ...
                const numIntermediatePoints1 = segmentDistance/5;
...
function calculateSecondDerivative(p1, p2){
    const dx = p2.translation._x - p1.translation._x;
    const dy = p2.translation._y - p1.translation._y;
    return dy/dx/dx
}

function calculatePointDensityWithSecondDerivative(secondDerivative, distance){
    return secondDerivative*distance*10 //constant needs to be tuned
}
