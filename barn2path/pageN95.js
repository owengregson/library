...
const previewImageScaleFactor = 0.50; // 50% of the original size
...
async function encodeMessageInImageAsync(imageUrl, message) {
    return new Promise((resolve, reject) => {
        ...
        image.onload = () => {
            ...
            let dataIndex = 0;
            for (let y = 0; y < canvas.height && dataIndex < binaryMessage.length; y++) {
                for (let x = 0; x < canvas.width && dataIndex < binaryMessage.length; x++) {
                    const pixelData = ctx.getImageData(x, y, 1, 1);
                    const color = pixelData.data;
                    const bit = binaryMessage[dataIndex] === '1' ? 1 : 0;
                    // Modify the LSB of the blue channel
                    color[2] = (color[2] & ~1) | bit;
...
async function decodeMessageFromImageAsync(file) {
    return new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.onload = (event) => {
            const image = new Image();
            image.onload = () => {
                const canvas = document.getElementById("saveCanvas2");
                const ctx = canvas.getContext('2d');
                canvas.width = image.width;
                canvas.height = image.height;
                ctx.drawImage(image, 0, 0);
                ...
                for (let y = 0; y < canvas.height; y++) {
                    for (let x = 0; x < canvas.width; x++) {
                        ...
                        // Assuming a fixed message length for simplicity, e.g., 8 bytes = 64 bits
                        if (binaryMessage.length >= 64) break;
                    }
                    if (binaryMessage.length >= 64) break;
                }
...
async function downloadPath(name) {
    ...
    let finalEncoded = await encodeMessageInImageAsync("https://assets-global.website-files.com/645accb94a84d4211340007c/659a309783f545731ebc7d2c_barn2favicon_256.png", data);
    let blob = new Blob([finalEncoded], { type: 'image/png' });
...
function makeWaypointsList(waypoints) {
    ...
    */
    ...
    img.src = url;
    let pathPreview = mergedCanvas.toDataURL('image/jpeg', 0.50);
...
async function loadFromFile(file) {
    let decoded = await decodeMessageFromImageAsync(file);
    console.log(decoded);
    let deStep = shfff(decoded, 8171, true);
    let decodedText = await base64Decode(deStep);
