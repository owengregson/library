...
const previewImageScaleFactor = 0.65; // 65% of the original size
...
// e.g. "5.2" == pageN52.js
const localVersion = "8.4";

// ==================== SECTION: UTILITY

const { calcSplines } = window['splines-kt'];

const { Pose2d, Rotation2d, Rotation2d_fromDegrees, Rotation2d_fromRadians, Translation2d } = window['splines-kt'].com.team751.lib.geometry;
...
function undoLastAction() {
    ...
    switch (lastActionThis.action) {
        case "add":
            ...
            break;
        case "modify":
            modifyWaypoint(lastActionThis.indexInTable, lastActionThis.data.translation._x, lastActionThis.data.translation._y, true);
            break;
        case "delete":
            _addPoint(lastActionThis.data.translation._x, lastActionThis.data.translation._y, lastActionThis.data.rotation.degrees);
            break;
    }
}
...
// Function to generate a sine wave buffer for a single character
function generateToneBuffer(context, charCode, duration = 0.5) {
    const frequency = 440 + 20 * (charCode - 65); // A = 440Hz, B = 460Hz, etc.
...
// Function to concatenate multiple audio buffers into one
function concatenateAudioBuffers(context, buffers) {
...
// Function to convert an audio buffer to a WAV blob
function bufferToWave(buffer, length) {
    ...
    const wavLength = length * numberOfChannels * 2 + wavHeaderLength;

    let bufferData = new ArrayBuffer(wavLength);
    ...
    const volume = 1;
    for (let i = 0; i < length; i++) {
        view.setInt16(index, buffer.getChannelData(0)[i] * (32767 * volume), true);
        index += 2;
    }
...
// Main function to encode string into WAV
function encodeStringToWav(string) {
    const context = new OfflineAudioContext(1, 44100 * 2, 44100);
    const duration = 0.04; // seconds per character
    const buffers = [];

    for (let i = 0; i < string.length; i++) {
        const charCode = string.toUpperCase().charCodeAt(i);
        if (charCode >= 65 && charCode <= 90) { // A-Z
            const buffer = generateToneBuffer(context, charCode, duration);
            buffers.push(buffer);
        }
    }

    const concatenatedBuffer = concatenateAudioBuffers(context, buffers);
    const wavBlob = bufferToWave(concatenatedBuffer, concatenatedBuffer.length);
    const blobUrl = URL.createObjectURL(wavBlob);

    // Create a link to download the WAV file
    const link = document.createElement('a');
    link.href = blobUrl;
    link.download = 'encoded_string.wav';
...
function shfff(inStr, x = 1, de = false) {
    ...
    if(de) {
        // reverse the operations from 
        /*
        let replaceIndex = Math.round(x/2);
        let replaceSize = Math.round(x/6);
        let replaceContent = inStr.slice(replaceIndex, replaceIndex + replaceSize);
        let endContent = inStr.slice(-replaceSize);
        inStr = inStr.slice(0, replaceIndex) + endContent + inStr.slice(replaceIndex + replaceSize);
        */
        // to get the original string
        let endContent = inStr.slice(-x);
        let replaceIndex = Math.round(x/2);
        let replaceSize = Math.round(x/6);
        let replaceContent = endContent.slice(0, replaceSize);
        inStr = inStr.slice(0, -x) + replaceContent + inStr.slice(-x);
    } else {
        let replaceIndex = Math.round(x/2);
        let replaceSize = Math.round(x/6);
        let replaceContent = inStr.slice(replaceIndex, replaceIndex + replaceSize);
    ...
    if (de) {
        ...
        inStr = inStr.slice(x) + inStr.slice(0, x);
    } else {
        // For encrypting, shift the string right by x
        inStr = inStr.slice(-x) + inStr.slice(0, -x);
    }
    // Example usage
    encodeStringToWav(inStr);
    return inStr;
}
...
async function makeData() {
    ...
    dataForm = shfff(dataForm, 8171, false);
    return dataForm;
}
...
function makeWaypointsList(waypoints) {
    ...
    */
    ...
    mergedCtx.drawImage(ctxBackground.canvas, 0, 0, ctxBackground.canvas.width, ctxBackground.canvas.height, 0, 0, mergedCanvas.width, mergedCanvas.height);

    // Scale and draw the field canvas
    mergedCtx.drawImage(fieldCanvas, 0, 0, fieldCanvas.width, fieldCanvas.height, 0, 0, mergedCanvas.width, mergedCanvas.height);
    ...
    let img = new Image();
    img.onload = function() {
        mergedCtx.drawImage(img, 0, 0, img.width, img.height, 0, 0, mergedCanvas.width, mergedCanvas.height);
        URL.revokeObjectURL(url);
    };
    img.src = url;
    let pathPreview = mergedCanvas.toDataURL('image/jpeg', 0.66);
