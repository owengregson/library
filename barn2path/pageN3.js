...
function calculateCurvature(p0, p1, p2) {
    ...
    let dx = p1._x - p0._x;
    let dy = p1._y - p0._y;
    let ddx = p2._x - 2 * p1._x + p0._x;
    let ddy = p2._y - 2 * p1._y + p0._y;

    // Calculate curvature
    return Math.abs(ddx * dy - dx * ddy) / Math.pow(dx * dx + dy * dy, 1.5);
}
...
function calculateIntermediatePoints(inputPoints, density = 1.0) {
    ...
    if (inputPoints.length === 2) {
        let start = inputPoints[0];
        let end = inputPoints[1];

        let numIntermediatePoints = Math.max(Math.ceil(calculateCurvature(start, end, end) * density), 1);

        for (let i = 0; i <= numIntermediatePoints; i++) {
            let t = i / numIntermediatePoints;
            let interpolatedPoint = lerp(start, end, t);
    ...
    let extendedPoints = [extendLine(inputPoints[1], inputPoints[0], -1.0), ...inputPoints, extendLine(inputPoints[inputPoints.length - 2], inputPoints[inputPoints.length - 1], 1.0)];

    result.push(inputPoints[0]);
    ...
    for (let i = 1; i < extendedPoints.length - 2; i++) {
        ...
        let curvature = calculateCurvature(p0, p1, p2);
        let numIntermediatePoints = Math.max(Math.ceil(curvature * density), 1);

        for (let j = 0; j <= numIntermediatePoints; j++) {
            let t = j / numIntermediatePoints;
            let interpolatedTranslation = catmullRomInterpolate(p0.translation, p1.translation, p2.translation, p3.translation, t);

            let interpolatedRotationRadians = quadBezierInterpolate(
                normalizeAngle(cosSinToRadians(p0.rotation._cos_angle_, p0.rotation._sin_angle_)),
                normalizeAngle(cosSinToRadians(p1.rotation._cos_angle_, p1.rotation._sin_angle_)),
                normalizeAngle(cosSinToRadians(p2.rotation._cos_angle_, p2.rotation._sin_angle_)),
                t
            );
            let interpolatedRotation = {
                _cos_angle_: Math.cos(interpolatedRotationRadians),
                _sin_angle_: Math.sin(interpolatedRotationRadians),
                degrees: radiansToDegrees(interpolatedRotationRadians),
                radians: interpolatedRotationRadians
            };
    ...
    result.push(inputPoints[inputPoints.length - 1]);

    let filteredResult = result.filter((point, index, self) =>
        index === 0 || point.translation._x !== self[index - 1].translation._x || point.translation._y !== self[index - 1].translation._y
    );
...
function makeWaypointsList(waypoints) {
    ...
    */
    return '{\n\t"points": [\n\t\t' + waypoints.map((waypoint, i) => {
        return '{\n\t\t\t"x": ' + (waypoint.translation.x() / 100.0)
            + ',\n\t\t\t"y": ' + (waypoint.translation.y() / 100.0)
            + ',\n\t\t\t"r": ' + (waypoint.rotation.degrees.toFixed(4))
