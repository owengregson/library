    ...
    for (let i = 1; i < extendedPoints.length - 1; i++) {
        ...
        if (currentRotation !== nextRotation || i === extendedPoints.length - 2) {
            ...
            for (let j = segmentStartIndex; j <= i; j++) {
                ...
                const numIntermediatePoints = Math.max(Math.ceil(segmentDistance * density), 1);
                // calculate numIntermediatePointsDer by the derivative curvature instead of the segment length
                const numIntermediatePointsDer = Math.max(Math.ceil(Math.abs((p0.rotation - p1.rotation) / (p1.rotation - p2.rotation)) * density), 1);

                for (let k = 0; k <= numIntermediatePointsDer; k++) {
                    let t = k / numIntermediatePointsDer;
