...
function calculateDistanceNoTranslation(p1, p2) {
...
function calculateSegmentCurvature(p0, p1, p2, p3) {
    // Sample several points along the segment to find the maximum curvature
    const samples = 10;
    let maxCurvature = 0;
    for (let i = 0; i <= samples; i++) {
        let t = i / samples;
        let curvature = calculateCurvature(p0, p1, p2, p3, t);
        maxCurvature = Math.max(maxCurvature, Math.abs(curvature));
    }
    return maxCurvature;
}

function calculateDensityBasedOnCurvatureAndLength(p0, p1, p2, p3) {
    // Calculate the segment length (you might want to adjust this to your actual calculation)
    const segmentLength = calculateDistanceNoTranslation(p1, p2);

    // Calculate the curvature of the segment
    const curvature = calculateSegmentCurvature(p0, p1, p2, p3);

    // Define a base density (points per cm) and adjust according to curvature
    const baseDensity = 0.05; // Base density: 1 point per 20 cm for straight lines
    const curvatureSensitivity = 500; // Adjust this factor based on how sensitive you want the density to curvature

    // Adjust density based on curvature (higher curvature -> higher density)
    const density = baseDensity + curvature * curvatureSensitivity;

    // Calculate the number of intermediate points (at least 1)
    let numPoints = Math.max(1, Math.round(segmentLength * density));
...
function calculateCurvature(p0, p1, p2, p3, t) {
    ...
    const dx_dt = -0.5 * p0._x + 0.5 * p2._x + 2 * (-p1._x + p2._x) * t + 1.5 * (p1._x - p2._x + 0.5 * (p3._x - p0._x)) * t * t;
    const dy_dt = -0.5 * p0._y + 0.5 * p2._y + 2 * (-p1._y + p2._y) * t + 1.5 * (p1._y - p2._y + 0.5 * (p3._y - p0._y)) * t * t;

    // Calculate the second derivatives of x and y with respect to t
    const ddx_ddt = -p1._x + p2._x + 3 * (p1._x - p2._x + 0.5 * (p3._x - p0._x)) * t;
    const ddy_ddt = -p1._y + p2._y + 3 * (p1._y - p2._y + 0.5 * (p3._y - p0._y)) * t;

    // Calculate the curvature using the formula for parametric curves
    const curvature = (dx_dt * ddy_ddt - dy_dt * ddx_ddt) / Math.pow((dx_dt * dx_dt + dy_dt * dy_dt), 1.5);
    ...
    for (let i = 1; i < extendedPoints.length - 1; i++) {
        ...
        if (currentRotation !== nextRotation || i === extendedPoints.length - 2) {
            ...
            for (let j = segmentStartIndex; j <= i; j++) {
                ...
                let p01 = inputPoints[i - 1];
                let p11 = inputPoints[i];
                let p21 = inputPoints[i + 1];
                let p31 = inputPoints[i + 2];

                console.log("\n\n new "+calculateDensityBasedOnCurvatureAndLength(p01, p11, p21, p31));

                let numIntermediatePoints = Math.max(Math.ceil(segmentDistance * density), 1);
                if(segmentDistance < 10) { numIntermediatePoints = 0; }

                console.log("old "+numIntermediatePoints+"\n\n");
